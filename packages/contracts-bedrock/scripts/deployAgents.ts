...
const main = async () => {
  ...
  const agentsDeployment = await deployProxy(
    'Agents',
    [admin, await treasury.getAddress()],
    deployer
  )
  ...
  const grantInflow = await treasury.grantRole(
  ...
  const tasksDeployment = await deployProxy(
    'Tasks',
    [admin, await agents.getAddress(), await treasury.getAddress()],
    deployer
  )
  ...
  const grantTaskModule = await agents.grantRole(
    taskModuleRole,
    await tasks.getAddress()
  )
  await grantTaskModule.wait()
  const grantSlasher = await agents.grantRole(
    slasherRole,
    await tasks.getAddress()
  )
  ...
  const payload = {
    ...
    treasuryImplementation:
      await treasuryDeployment.implementation.getAddress(),
    ...
    subscriptionsImplementation:
      await subscriptionsDeployment.implementation.getAddress(),
